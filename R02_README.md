...
L'application est connectée au projet Firebase suivant :

| Paramètre        | Valeur                                    |
|------------------|-------------------------------------------|
| **Projet**       | `inventaire-materiel-a7b8c`               |
| **Base de données** | Firestore (Europe West)                |
| **Collection**   | `eps` > document `data`                   |
...
service cloud.firestore {
  match /databases/{database}/documents {
    match /eps/data {
      allow read, write: if true; // accès libre sur réseau local
      // Pour restreindre : if request.auth != null;
    }
  }
}
```

> Si vous souhaitez restreindre l'accès à l'équipe EPS uniquement, activez **Firebase Authentication** (connexion par e-mail) et remplacez `if true` par `if request.auth != null`.
